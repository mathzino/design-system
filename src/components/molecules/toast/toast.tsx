...
const Toast: React.FC<ToastProps> = ({ variant, position, ...props }) => {
   ...
   return (
      <div>
         <Button size={ESizeButton.lg} variant={EVariantButton.default} children="Open" onClick={toggleToast} />
         ...
         <div className={`h-fit top-[34px] bottom-0 fixed -translate-y-1/2 rounded-xl transition-all duration-[400ms] z-40 ${cn(toastVariants({ variant, position }))} ${toast ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
            <div className="flex items-center justify-center gap-2 mx-[9px] my-1.5">
               ...
               <button onClick={toggleToast} className={`ml-2 flex items-center justify-center w-8 h-8 rounded-full ${variant === 'default'? 'invisible w-0 h-0 ml-0' : variant === 'success' ? 'hover:bg-[#ccdad2]' : 'hover:bg-[#ffaeae]'}`}>
