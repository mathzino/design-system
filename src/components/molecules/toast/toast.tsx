...
export type TVariantToast = "default" | "danger" | "success";

export type TPositionToast = "left" | "middle" | "right";
...
const Toast: React.FC<ToastProps> = ({ variant, position, condition=true, ...props }) => {
   ...
   return (
      <div>
         <div className={`h-fit top-[34px] bottom-0 fixed -translate-y-1/2 rounded-xl transition-all duration-[400ms] z-40 ${cn(toastVariants({ variant, position }))} ${toast ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
            <div className='flex items-center justify-center gap-2 mx-[9px] my-1.5'>
               <img src={variant === 'success' ? successIcon : variant === 'danger' ? errorIcon : loadingIcon} alt="condition-toast-icon" className={`w-6 h-6 ${variant === 'default'? 'animate-spin ml-[9px]' : ''}`} />
               <p className="text-xl mb-[2px] font-medium">
                  {variant === 'success' ? 'Succeed' : variant === 'danger' ? 'Oops, something went wrong' : 'Loading...'}
               </p>
               <button onClick={toggleToast} className={`ml-2 flex items-center justify-center rounded-full ${variant === 'default'? 'invisible w-0 h-0 ml-0' : variant === 'success' ? 'hover:bg-[#ccdad2] w-8 h-8' : 'hover:bg-[#ffaeae] w-8 h-8'}`}>
